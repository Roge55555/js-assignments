...
function getFizzBuzz(num) {
    let a="";
    for (let i=0;i<100;i++){
    if(num%3===0&&i===3)a+="Fizz";
    if(num%5===0&&i===5)a+="Buzz";
    if(i===num&&a==="")a+=i;
    }
    return(a);
}
...
function findFirstSingleChar(str) {
    ...
    for(let i=0;i<str.length;i++)
        a[str[i].charCodeAt(0)]+=1;
    for(let i=0;i<str.length;i++)
        if(a[str[i].charCodeAt(0)]<2){b=str[i];break;}
        if(b==="")return(null);
    else return(b);
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    let x="";
    if(isStartIncluded===true)x+="[";
    else x+="(";
    if(a>b){x+=b;x+=", ";x+=a;}
    else{x+=a;x+=", ";x+=b;}
    if(isEndIncluded===true)x+="]";
    else x+=")";
    return(x);
}
...
function isBracketsBalanced(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        else if (ans.length >= 0 && str[i] !== ans[ans.length - 1]) {return false;}
